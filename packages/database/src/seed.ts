...
async function seed() {
  ...
  const [codeReviewer] = await db
    ...
    .values({
      ...
      systemPrompt: "You are an expert code reviewer. Analyze pull requests for bugs, security vulnerabilities, style issues, and performance problems. Provide actionable feedback.",
      model: "gpt-4",
      provider: "openai",
      temperature: "0.3",
      maxTokens: 4096,
  ...
  const [researchAssistant] = await db
    ...
    .values({
      ...
      systemPrompt: "You are a research assistant. Search the web, gather information, and produce comprehensive summaries with citations.",
      model: "gpt-4",
      provider: "openai",
      temperature: "0.7",
      maxTokens: 2048,
  ...
  const [bugHunter] = await db
    ...
    .values({
      ...
      systemPrompt: "You are a bug hunting specialist. Analyze code and logs to identify, reproduce, and suggest fixes for bugs.",
      model: "claude-3-opus",
      provider: "anthropic",
      temperature: "0.2",
      maxTokens: 8192,
  ...
  const [codeRunnerTool] = await db
    ...
    .values({
      ...
      schema: {
        ...
        output: { type: "object", properties: { stdout: { type: "string" }, stderr: { type: "string" }, exitCode: { type: "integer" } } },
      },
      isBuiltin: true,
      isPublic: true,
    })
  ...
  const [httpRequestTool] = await db
    ...
    .values({
      ...
      schema: {
        ...
        output: { type: "object", properties: { status: { type: "integer" }, body: { type: "string" }, headers: { type: "object" } } },
      },
      isBuiltin: true,
      isPublic: true,
    })
  ...
  console.log(`Created tools: ${webSearchTool.name}, ${codeRunnerTool.name}, ${httpRequestTool.name}`);
  ...
  const [listing1] = await db
    ...
    .values({
      ...
      description: "An advanced code review agent that integrates with GitHub, GitLab, and Bitbucket. Supports 15+ programming languages with custom rule sets.",
  ...
  const [listing2] = await db
    ...
    .values({
      name: "PR Review Pipeline",
      description: "Complete automated PR review workflow that lints, tests, reviews code, and generates comprehensive reports.",
  ...
  const [exec1] = await db
    ...
    .values({
      ...
      output: {
        ...
        issues: [
          { severity: "high", file: "src/core/runner.ts", line: 156, message: "SQL injection vulnerability" },
  ...
  const [exec3] = await db
    ...
    .values({
      ...
      steps: [
        { name: "lint", status: "running", duration: 1500 },
      ],
      startedAt: new Date(Date.now() - 1500),
    })
    .returning();

  console.log(`Created executions: ${exec1.id.slice(0, 8)}..., ${exec2.id.slice(0, 8)}..., ${exec3.id.slice(0, 8)}...`);
