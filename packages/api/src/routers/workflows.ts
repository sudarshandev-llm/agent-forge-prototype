...
export const workflowRouter = router({
  list: authProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (input.search) {
        ...
        list = list.filter(
          (w) =>
            w.name.toLowerCase().includes(lower) || w.description.toLowerCase().includes(lower),
        );
      }
  ...
  create: authProcedure.input(createWorkflowSchema).mutation(async ({ ctx, input }) => {
  ...
  update: authProcedure.input(updateWorkflowSchema).mutation(async ({ ctx, input }) => {
