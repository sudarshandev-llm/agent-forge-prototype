...
const publishListingSchema = z.object({
  ...
  pricing: z
    .union([
      z.literal('free'),
      z.object({ amount: z.number(), currency: z.string().default('USD') }),
    ])
    .default('free'),
...
function recalcRating(listingId: string): void {
  ...
  listing.averageRating =
    listingReviews.length > 0 ? Math.round((totalRating / listingReviews.length) * 10) / 10 : 0;
...
export const marketplaceRouter = router({
  ...
  getListingById: publicProcedure.input(z.object({ id: z.string() })).query(async ({ input }) => {
    const listing = listings.get(input.id);
    if (!listing) throw new TRPCError({ code: 'NOT_FOUND', message: 'Listing not found' });
    return listing;
  }),

  publish: authProcedure.input(publishListingSchema).mutation(async ({ ctx, input }) => {
  ...
  addReview: authProcedure.input(addReviewSchema).mutation(async ({ ctx, input }) => {
