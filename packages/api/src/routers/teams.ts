...
export const teamRouter = router({
  ...
  getById: authProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const isMember = team.ownerId === ctx.userId || team.members.some((m) => m.userId === ctx.userId);
  ...
  addMember: authProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const member: TeamMember = { userId: input.userId, role: input.role, joinedAt: new Date().toISOString() };
