...
const caller = (ctx: TRPCContext) => agentRouter.createCaller(ctx);
...
describe('agentRouter', () => {
  ...
  describe('list', () => {
    ...
    it('should only return agents owned by the user', async () => {
      ...
      const ctx2 = createCtx({
        userId: 'user-2',
        user: { id: 'user-2', email: 'other@test.com', name: 'Other', roles: ['user'] },
      });
  ...
  describe('update', () => {
    it('should update an agent', async () => {
      ...
      const updated = await caller(ctx).update({
        id: created.id,
        name: 'Updated',
        temperature: 0.5,
      });
    ...
    it('should throw NOT_FOUND for non-existent id', async () => {
      const ctx = createCtx();
      await expect(caller(ctx).update({ id: 'non-existent', name: 'Nope' })).rejects.toThrow(
        'Agent not found',
      );
    });
    ...
    it("should throw FORBIDDEN for another user's agent", async () => {
      ...
      const created = await caller(ctx1).create({ name: 'My Agent' });
      await expect(caller(ctx2).update({ id: created.id, name: 'Hacked' })).rejects.toThrow(
        'FORBIDDEN',
      );
    });
  });
  ...
  describe('run', () => {
    ...
    it('should throw NOT_FOUND for non-existent agent', async () => {
      const ctx = createCtx();
      await expect(caller(ctx).run({ id: 'non-existent', input: 'Hello' })).rejects.toThrow(
        'Agent not found',
      );
    });
  });
  ...
  describe('getExecutions', () => {
    ...
    it('should throw NOT_FOUND for non-existent agent', async () => {
      const ctx = createCtx();
      await expect(caller(ctx).getExecutions({ agentId: 'non-existent' })).rejects.toThrow(
        'Agent not found',
      );
    });
  });
